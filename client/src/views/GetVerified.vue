...
async function handleVerify(e:any){
    ...
    try {
        ...
        if(parseRes.error){
            ...
            wait.value="cursor-pointer bg-gray-700"
            ...
            <div class="my-1 flex flex-col items-center w-full">
                <div class="flex flex-col w-full my-4 max-sm:my-2">
                    ...
                    <form class="flex flex-col items-center" @submit="handleVerify">
                        <input type="email" name="email" class="mt-2 border-gray-500 placeholder:text-gray-700 border-[1px] bg-white rounded-lg focus:outline-1 focus:outline-gray-700 w-[100%] py-2 px-4 placeholder:text-sm text-sm" placeholder="Enter your email" required/>
                    ...
                    <div @click="router.push('/signin')" class="text-gray-700 font-semibold mt-4 text-center text-sm max-sm:text-xs cursor-pointer" title="Sign in">I have an account?</div>
                </div>
            </div>
        </div>
    </div>
